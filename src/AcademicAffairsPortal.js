...
function AcademicAffairsPortal() {
  ...
  const handleClassSubmit = async (e) => {
    ...
    try {
      if (mode === 'add') {
        await academicAPI.createClass(data.semesterId, data);
      } else {
        await academicAPI.updateClass(data.id, data);
      }
  ...
  const handleStudentSubmit = async (e) => {
    ...
    try {
      if (mode === 'add') {
        ...
        await academicAPI.updateStudent(data.id, data);
      }
  ...
  const fetchLecturers = async () => {
    try {
      const data = await academicAPI.getLecturers();
  ...
  const handleLecturerSubmit = async (e) => {
    ...
    try {
      if (mode === 'add') {
        await academicAPI.createLecturer(data);
      } else {
        await academicAPI.updateLecturer(data.id, data);
      }
  ...
  const handleDeleteLecturer = async (id) => {
    if (window.confirm('Are you sure you want to delete this lecturer?')) {
      try {
        await academicAPI.deleteLecturer(id);
  ...
  const fetchEnrollments = async () => {
    try {
      const data = await academicAPI.getEnrollments();
  ...
  const handleEnrollmentSubmit = async (e) => {
    ...
    try {
      await academicAPI.createEnrollment(enrollmentDialog.data);
  ...
  const handleUnenroll = async (enrollmentId) => {
    if (window.confirm('Are you sure you want to unenroll this student?')) {
      try {
        await academicAPI.deleteEnrollment(enrollmentId);
  ...
  const fetchAssignments = async () => {
    try {
      const data = await academicAPI.getAssignments();
  ...
  const handleAssignmentSubmit = async (e) => {
    ...
    try {
      await academicAPI.createAssignment(assignmentDialog.data);
  ...
  const fetchLogs = async () => {
    try {
      const data = await academicAPI.getLogs();
      setLogs(data.logs || []);
    } catch (error) {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mt: 4 }}>
      ...
      {message && <Alert sx={{ mb: 2, width: '100%', maxWidth: 1200 }} severity="info" onClose={() => setMessage('')}>{message}</Alert>}
