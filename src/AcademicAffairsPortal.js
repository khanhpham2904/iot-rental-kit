...
import { mockSemesters, mockStudents, mockLecturers, mockEnrollments, mockAcademicAssignments, mockKits } from './mocks';
...
function AcademicAffairsPortal({ user, onLogout }) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setAssignments(mockAcademicAssignments);
        ...
        <Content style={{ 
          ...
          >
            <AnimatePresence mode="wait">
              ...
              >
                {selectedKey === 'dashboard' && <DashboardContent semesters={semesters} students={students} lecturers={lecturers} enrollments={enrollments} assignments={assignments} kits={kits} iotSubjects={iotSubjects} />}
                ...
                {selectedKey === 'enrollments' && <EnrollmentManagement enrollments={enrollments} setEnrollments={setEnrollments} enrollmentModal={enrollmentModal} setEnrollmentModal={setEnrollmentModal} enrollmentForm={enrollmentForm} students={students} semesters={semesters} />}
                {selectedKey === 'assignments' && <AssignmentManagement assignments={assignments} setAssignments={setAssignments} assignmentModal={assignmentModal} setAssignmentModal={setAssignmentModal} assignmentForm={assignmentForm} lecturers={lecturers} semesters={semesters} />}
...
// Dashboard Component
const DashboardContent = ({ semesters, students, lecturers, enrollments, assignments, kits, iotSubjects }) => {
        ...
        <Col xs={24} lg={12}>
          <motion.div variants={cardVariants} initial="hidden" animate="visible" whileHover="hover">
            <Card 
              ...
              style={{ borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.1)' }}
            >
              <Tabs defaultActiveKey="enrollments" size="small">
                <Tabs.TabPane tab="Enrollments" key="enrollments">
                  <List
                    size="small"
                    ...
                    renderItem={(item, index) => (
                      <List.Item>
                        <List.Item.Meta
                          ...
                          title={item.title}
                          description={`Assigned by ${item.lecturerName}`}
                        />
                        <div style={{ textAlign: 'right' }}>
                          <div style={{ fontSize: '12px', color: '#666' }}>
...
const EnrollmentManagement = ({ enrollments, setEnrollments, enrollmentModal, setEnrollmentModal, enrollmentForm, students, semesters }) => (
  <div>
    <motion.div variants={cardVariants} initial="hidden" animate="visible" whileHover="hover">
      <Card 
...
const AssignmentManagement = ({ assignments, setAssignments, assignmentModal, setAssignmentModal, assignmentForm, lecturers, semesters }) => {
  const handleAddAssignment = () => {
    assignmentForm.resetFields();
    setAssignmentModal({ visible: true, data: {} });
  };
  ...
  const handleEditAssignment = (record) => {
    assignmentForm.setFieldsValue({
      ...
      assignmentDate: record.assignmentDate ? dayjs(record.assignmentDate) : null,
      status: record.status
    });
    setAssignmentModal({ visible: true, data: record });
  };

  const handleDeleteAssignment = (record) => {
    Modal.confirm({
      title: 'Delete Assignment',
      content: `Are you sure you want to delete the assignment for ${record.lecturerName}?`,
      okText: 'Delete',
      okType: 'danger',
      cancelText: 'Cancel',
      onOk: () => {
  ...
  const handleAssignmentSubmit = () => {
    assignmentForm.validateFields().then(values => {
      const newAssignment = {
        id: assignmentModal.data.id || Date.now(),
        lecturerId: values.lecturerId,
        lecturerName: lecturers.find(l => l.id === values.lecturerId)?.name || 'Unknown Lecturer',
        semesterId: values.semesterId,
        semesterName: semesters.find(s => s.id === values.semesterId)?.name || 'Unknown Semester',
        assignmentDate: values.assignmentDate ? values.assignmentDate.format('YYYY-MM-DD') : new Date().toISOString().split('T')[0],
        status: values.status || 'Active'
      };

      if (assignmentModal.data.id) {
        // Edit existing assignment
        setAssignments(prev => prev.map(item => 
          item.id === assignmentModal.data.id ? newAssignment : item
        ));
      ...
      setAssignmentModal({ visible: false, data: {} });
      assignmentForm.resetFields();
    });
  };
  ...
  return (
    <div>
          ...
          <Table
            ...
            columns={[
              ...
              },
              {
                ...
                render: (_, record) => (
                  <Space>
                    <Button type="primary" size="small" icon={<EditOutlined />} onClick={() => handleEditAssignment(record)}>
                      Edit
                    </Button>
                    <Button type="primary" danger size="small" icon={<DeleteOutlined />} onClick={() => handleDeleteAssignment(record)}>
                      Remove
                    </Button>
                  </Space>
                ),
      ...
      {/* Assignment Modal */}
      <Modal
        title={assignmentModal.data.id ? 'Edit Assignment' : 'Add Assignment'}
        ...
        okText={assignmentModal.data.id ? 'Update' : 'Add'}
        cancelText="Cancel"
        width={600}
      >
        <Form
