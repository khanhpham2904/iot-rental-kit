...
function AdminRefundApprovalPage() {
  ...
  const handleApprove = async () => {
    ...
    try {
      const data = await refundAPI.approveRefundRequest(selectedRequest.id, damageAssessment);
            ...
            <Box>
              ...
              {qrCode && (
                <Box sx={{ mt: 2, textAlign: 'center' }}>
                  <Typography variant="h6" gutterBottom>QR Code Generated</Typography>
                  <Box sx={{ p: 2, bgcolor: 'white', display: 'inline-block', border: '1px solid #ccc' }}>
                    <Typography variant="h4" sx={{ fontFamily: 'monospace' }}>
                      {qrCode}
                    </Typography>
                  </Box>
                  <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
                    Share this QR code with the user for refund processing
                  </Typography>
                </Box>
              )}
            </Box>
